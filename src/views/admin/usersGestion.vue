  ...
  <div class="dashboard admin-dashboard">
    ...
    <div class="section-card user-management">
      ...
      <div class="user-tabs">
        ...
        <button
          @click="userActiveTab = 'DISABLED'"
          :class="['tab-btn', { active: userActiveTab === 'DISABLED' }]"
        >
          <i class="fas fa-ban"></i>
          Disabled <span class="tab-count">{{ disabledUsersCount }}</span>
        </button>

        <button
          @click="userActiveTab = 'SUSPENDED'"
          :class="['tab-btn', { active: userActiveTab === 'SUSPENDED' }]"
        >
          <i class="fas fa-exclamation-triangle"></i>
          Suspended <span class="tab-count">{{ suspendedUsersCount }}</span>
        </button>
      </div>
...
const disableUser = async id => {
  ...
  await axios.put(`${API}/${id}/blockUser`, {}, { headers: { Authorization: `Bearer ${token}` } })
  await fetchUsers()
}

const activateUser = async id => {
  await axios.put(`${API}/${id}/unblockUser`, {}, { headers: { Authorization: `Bearer ${token}` } })
  await fetchUsers()
}
...
const addNewUser = async () => {
  await axios.post(API, newUser.value, { headers: { Authorization: `Bearer ${token}` } })
  showAddUserModal.value = false
  newUser.value = { username: '', email: '', password: '', role: 'ROLE_CLIENT' }
  await fetchUsers()
}
...
const filteredUsers = computed(() => {
  ...
  if (userActiveTab.value !== 'ALL') list = list.filter(u => u.status === userActiveTab.value)
  return list
})

const totalUsers = computed(() => users.value.length)
const disabledUsersCount = computed(() => users.value.filter(u => u.status === 'DISABLED').length)
const suspendedUsersCount = computed(() => users.value.filter(u => u.status === 'SUSPENDED').length)
...
let timer = null
onMounted(() => { updateTime(); timer = setInterval(updateTime, 60000); fetchUsers() })
